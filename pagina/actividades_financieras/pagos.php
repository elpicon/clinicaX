
<?php include '../layout/header.php';

//$branch_id = $_GET['id'];
?>

    <!-- Font Awesome -->

    <link rel="stylesheet" href="../layout/plugins/datatables/dataTables.bootstrap.css">
    <link rel="stylesheet" href="../layout/dist/css/AdminLTE.min.css">
    <link rel="stylesheet" href="../layout/plugins/select2/select2.min.css">
    <link rel="stylesheet" href="css/actividades.css" type="text/css">
    <!-- AdminLTE Skins. Choose a skin from the css/skins
        folder instead of downloading all of them to reduce the load. -->
    <link rel="stylesheet" href="../layout/dist/css/skins/_all-skins.min.css">
  <body class="nav-md">
                                        <?php 
//    if ($usuario=="si") {
      # code...
    
?>
    <div class="container body">
      <div class="main_container">
        <?php include '../layout/main_sidebar.php';?>

        <!-- top navigation -->
            <?php include '../layout/top_nav.php';?>      <!-- /top navigation -->
                      <style>
                label{

                color: black;
                }
                li {
                  color: white;
                }
                ul {
                  color: white;
                }
                #buscar{
                  text-align: right;
                }
                      </style>

        <!-- page content -->
            <div class="right_col" role="main">

                <?php
                  ?>

                          <?php
                        
             //         if ($guardar=="si") {
                    
                      ?>

                  <?php
               //       }
                      ?>

                  <!-- Date range -->

                  <!--end of modal-->
                <div class="box-header">
                <h3 class="htitle">Pagos</h3>
                </div><!-- /.box-header -->
                <a class="btn btn-plantilla3" href="../actividades_financieras/<?php  echo "pago_agregar.php";?>"     role="button"><i class="glyphicon glyphicon-plus"></i> Agregar</a>   
      

                <div class="box-body " >
                  <table id="example2" class="table table-bordered table-striped">
                    <thead>
                        <tr class="encabezado">

                        <th>Paciente</th>
                        <th>Médico</th>
                        <th>Fecha</th>
                        <th>Total</th>
                        <th  style="width:120px;"> Acciones </th>

                                  </tr>
                    </thead>
                    <tbody>
                            <?php

                            $auto="";
                            //select m.nombre as  medico,p.nombre as  paciente,c.fecha,c.observaciones,c.estado_cita,c.id_cita from pedidos c inner join usuario m on c.id_medico = m.id inner join usuario p on p.id = c.id_cliente
                              // $branch=$_SESSION['branch'];
                                $query=mysqli_query($con,"select m.nombre as  medico,p.nombre as  paciente,c.id_pedido,c.fecha from pedidos c inner join usuario m on c.id_medico = m.id inner join usuario p on p.id = c.id_cliente  ")or die(mysqli_error());

                    
                              while($row=mysqli_fetch_array($query)){

                          $id_pedido=$row['id_pedido'];

                              $sum=0;
                            $igv=0;   
                            $sub=0;   
                            $sub_igv=0;
                          //
                              $query1=mysqli_query($con,"select * from detalles_pedido d inner join procedimiento_pago p on d.id_producto = p.id_procedimiento_pago 
                                  where  id_pedido='$id_pedido'   ")or die(mysqli_error());

                    
                              while($row1=mysqli_fetch_array($query1)){
                                $sub=$sub+$row1['precio_venta']*$row1['cantidad'];}?>
                      <tr style="background: white;">
                          <td><?php echo $row['paciente'];?></td> 
                          <td><?php echo $row['medico'];?></td> 

        
                          <td><?php echo $row['fecha'];?></td>     
  
                          <td><?php echo $sub;?></td> 
                                        <td>
                                <?php
                            ?>
                            <a class="btn btn-plantilla" href="<?php  echo "../boletas_ventas/generar_boleta.php? id_pedido=$id_pedido";?>"  target="_blank"  role="button">Factura</a>
                            <a class="btn btn-danger" href="<?php  echo "../paciente/eliminar_cita.php?id_cita=$cid";?>"  onClick="return confirm('¿Está seguro de que quieres eliminar??');"   role="button"><i class="glyphicon glyphicon-trash" ></i></a>
                                  <?php
                                        
                                        ?>
                                </td>  
                      </tr>

                          <?php }?>
                    </tbody>
                  </table>
                </div><!-- /.box-body -->

              </div><!-- /.col -->

        <!-- /page content -->

        <?php include '../layout/footer.php';?>
      </div>
    </div>

                <?php include '../layout/datatable_script.php';?>



                            <script>
                            $(document).ready( function() {
                                    $('#example2').dataTable( {
                                    "language": {
                                      "paginate": {
                                          "previous": "anterior",
                                          "next": "posterior"
                                        },
                                        "search": "Buscar:",


                                      },

                                      "info": false,
                                      "lengthChange": false,
                                      "searching": false,


                      "searching": true,
                                    }

                                  );
                                  } );
                        </script>
                                                            <?php 
                    // }    
                    ?>



    <!-- /gauge.js -->
  </body>
</html>
