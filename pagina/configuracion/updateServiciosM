...
//echo "\nSI ". $bufferSI;
 ...
 //  echo "b= ". $b;
   if($b==0){
        mysqli_query($con,"UPDATE `servicios_grupos` SET `habilitacion_citas`='0'  WHERE codigo='".$grupo."'")or die(mysqli_error());
       //echo "<br>entro b 0= ". $datoFlag[0];
   }else{
        mysqli_query($con,"UPDATE `servicios_grupos` SET `habilitacion_citas`='1'  WHERE codigo='".$grupo."'")or die(mysqli_error());
...
if($a==1){
     mysqli_query($con,"UPDATE `servicios_h` SET `HABILITADO`='NO'  WHERE $bufferNO")or die(mysqli_error());
               //echo "Affected rows NO(INSERT): %d\n", mysqli_affected_rows($link);

}
 if($b==1){
    mysqli_query($con,"UPDATE `servicios_h` SET `HABILITADO`='SI'  WHERE $bufferSI")or die(mysqli_error());
